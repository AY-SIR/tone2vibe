    ...
    try {
      ...
      if (error) {
        if (error.message.includes('Invalid login credentials')) {
          toast.error("Invalid email or password. If you signed up with Google, please use 'Continue with Google'.", {
            duration: 6000,
          });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-auto no-scrollbar border-0 rounded-xl sm:max-w-[425px] w-[90%] mx-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {currentView === 'auth' ? 'Hello! Access your account or get started.' : 'Reset your password'}
          </DialogDescription>
        </DialogHeader>
