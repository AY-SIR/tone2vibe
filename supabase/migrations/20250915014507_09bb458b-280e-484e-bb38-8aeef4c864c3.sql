-- Fix the migration by creating proper constraints
-- Remove USD/international support and create India-only setup

-- First, let's create a unique constraint for plans
DROP TABLE IF EXISTS vpn_detections;
ALTER TABLE profiles DROP COLUMN IF EXISTS is_vpn_user;
ALTER TABLE profiles DROP COLUMN IF EXISTS last_ip_check;
ALTER TABLE profiles DROP COLUMN IF EXISTS ui_language;

-- Create unique constraint for plans
ALTER TABLE plans DROP CONSTRAINT IF EXISTS plans_name_currency_key;
ALTER TABLE plans ADD CONSTRAINT plans_name_currency_key UNIQUE (name, currency);

-- Remove USD pricing and add only INR
DELETE FROM plans WHERE currency != 'INR';

-- Add Indian-only plans
...
-- Add language preference (Hindi/English)
...
-- Remove Stripe/USD related data
